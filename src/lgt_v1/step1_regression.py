...
X1_train, X1_test, y1_train, y1_test = train_test_split(ueg_df.ix[:, 0:27], ueg_df.ix[:,27], test_size=0.3, random_state=30)
X2_train, X2_test, y2_train, y2_test = train_test_split(neg_ueg_df.ix[:, 0:27], neg_ueg_df.ix[:,27], test_size=0.2, random_state=30)
...
#2018/12/20
# 1.调整event_id(从src/ltmp_vars/12_18读取)，保留
clock_v1 = open("src/ltmp_vars/12_18/ueg_df.pkl", 'rb')
ueg_df = pickle.load(clock_v1)
clock_v1.close()
def adjust_eventidseq(ueg_df):
    tmp = ueg_df['event_id'].copy()
    del ueg_df['event_id']
    ueg_df['event_id'] = tmp
    return ueg_df
ueg_df = adjust_eventidseq(ueg_df)
neg_ueg_df = adjust_eventidseq(neg_ueg_df)
# feature_v1
def preprosess_df(ueg_df):
    ...
    return ueg_df
ueg_df = preprosess_df(ueg_df)
neg_ueg_df = preprosess_df(neg_ueg_df)
ueg_df['class'] = 1
neg_ueg_df['class'] = 0
#attention: neg_ueg_df其他列还是float型
neg_ueg_df = neg_ueg_df[neg_ueg_df.columns].astype(float)
# 2. 加载昨天的模型 [:, 0:29]
X1_train, X1_test, y1_train, y1_test = train_test_split(ueg_df.iloc[:, 0:29], ueg_df.iloc[:,29], test_size=0.3, random_state=33)
X2_train, X2_test, y2_train, y2_test = train_test_split(neg_ueg_df.iloc[:, 0:29], neg_ueg_df.iloc[:,29], test_size=0.3, random_state=33)
...
tr1=np.random.normal(mu,sigma,91184)
X1_train['rand'] = tr1
t1=np.random.normal(mu,sigma,39080)
X1_test['rand'] = t1

tr2=np.random.normal(mu,sigma,79137)
X2_train['rand'] = tr2
t2=np.random.normal(mu,sigma,33916)
X2_test['rand'] = t2
